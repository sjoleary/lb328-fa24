...
Keep in mind that we have data sets from two locations (Rand Brook and Schoolhouse Brook) for three different sampling seasons from multiple years.
...
This is helpful but our sample sizes differ among locations so we will want to determine the relative abundance for each location.

We can do this by dividing the number of individuals in a certain group by the total number of individuals sampled at that location.
...
Look at each data set separately to identify patterns and then start making comparisons within and among sites.
...
* are there seasonabl patterns? in one location? in both locations?
...
Other than a massive table with all the results, is there a good way to visualize this data set?
